...
vi.mock('@uiw/react-codemirror', () => ({
  default: ({ onChange }: any) => <textarea data-testid="codemirror-mock" onChange={(e) => onChange && onChange(e.target.value)} />
}));
...
import { RunnableCardComponent, CodeChallengeCardComponent } from './InteractiveCards';
...
describe('Code Challenge Card', () => {
  it('runs multiple tests and shows Lihat Solusi after 3 failures', async () => {
    ...
    fireEvent.change(screen.getByTestId('codemirror-mock'), { target: { value: 'public class Main { } // attempt 1' } });
    fireEvent.click(btn);
    await screen.findByText(/Berhenti sebentar/i, {}, { timeout: 3000 });

    // Fail 2
    fireEvent.change(screen.getByTestId('codemirror-mock'), { target: { value: 'public class Main { } // attempt 2' } });
    fireEvent.click(btn);
    await screen.findByText(/Jelaskan dengan kata-katamu/i, {}, { timeout: 3000 });

    // We must provide reflection text to get past Fail 2
    fireEvent.change(screen.getByPlaceholderText(/Ketik minimal 20 karakter.../), { target: { value: 'ini adalah refleksi yang cukup panjang lebih dari 20 karakter' } });

    // Fail 3
    fireEvent.change(screen.getByTestId('codemirror-mock'), { target: { value: 'public class Main { } // attempt 3' } });
    fireEvent.click(btn);
